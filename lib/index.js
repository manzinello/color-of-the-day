...
function color() {
	...
	request(COLORSTROLOGY, (error, response, body) => {
		...
		if (response && response.statusCode == 200) {
...

			{
				...
				var keyLogons = span.getElementsByTagName(DOM_SPAN);
				keyLogon = keyLogons[0].textContent.trim() + ", " + keyLogons[1].textContent.trim() + ", " + keyLogons[2].textContent.trim();

			}

			if (date && todayColor && todayColorHex && keyLogon && todayColorHex.length === 7 && todayColorHex.substring(0, 1) === HASHTAG) {
				printColor(date, keyLogon, todayColor, todayColorHex);
			} else {
...
function printColor(date, keyLogon, todayColor, todayColorHex) {
	console.log(DIVIDER);
	console.log(date);
	console.log(keyLogon);
